...
def main(args):
    ...
    os.system("mkdir -p {}".format(path_to_11_output))
    ...
    for file in tqdm(all_read_files):
        if os.path.isfile(os.path.join(path_to_11_output, file.name.replace(".sorted.csv", ".read_classification.csv"))) == False:
            ...
            ##### save the results
            resdf.to_csv(os.path.join(path_to_11_output, "{}.candi_reads.csv".format(file.name.split(".")[0])), index = False)
            tmpdf.to_csv(os.path.join(path_to_11_output, file.name.replace(".sorted.csv", ".read_classification.csv")))
        else:
    ...
    countdf.to_csv(os.path.join(path_to_11_output, "all_count.csv"))
