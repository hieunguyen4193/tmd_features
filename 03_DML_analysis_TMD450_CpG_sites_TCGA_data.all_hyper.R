...
# for (input.cancer.class in c("Liver")){
  ...
  if (file.exists(file.path(path.to.03.output, "DMPs.xlsx")) == FALSE){
    ...
    mean.group1 <- data.frame(mean.group1 = rowMeans(maindf[, group1])) %>% rownames_to_column("cpg")
    mean.group2 <- data.frame(mean.group2 = rowMeans(maindf[, group2])) %>% rownames_to_column("cpg")
    ...
    DMPs <- merge(DMPs, mean.group1, by.x = "TCGA_CpG", by.y = "cpg")
    DMPs <- merge(DMPs, mean.group2, by.x = "TCGA_CpG", by.y = "cpg")
    DMPs.raw <- DMPs
