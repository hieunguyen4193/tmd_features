...
library(ggpubr)
pcadf$Label <- factor(pcadf$Label, levels = c(input.cancer.class, "Control"))
pcadf %>% ggplot(aes(x = PC1, y = PC2, color = Label)) + geom_point(size = 1) + scale_color_manual(values = c("red", "grey")) + theme_pubr(base_size = 15)
```
...
check.metdf <- getData(meth)[,c("chr", "start")] %>% as.data.frame() %>%
  rowwise() %>%
  mutate(cpg = sprintf("%s_%s", chr, start))
intersect(cpg450df$cpg, check.metdf$cpg)
```
